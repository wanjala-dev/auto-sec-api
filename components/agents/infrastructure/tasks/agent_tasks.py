...
@shared_task(bind=True, soft_time_limit=AGENT_SOFT_TIME_LIMIT, time_limit=AGENT_TIME_LIMIT, name='infrastructure.ai.agents.tasks.run_agent_execution')
def run_agent_execution(self, execution_id: str) -> dict[str, Any]:
    ...
    if (
        ...
        logger.info(
            "run_agent_execution skip already-running execution_id=%s "
            "owner_task_id=%s incoming_task_id=%s",
            execution_id, execution.task_id, incoming_task_id,
        )
        return {
            'success': False,
            'execution_id': str(execution.id),
    ...
    try:
        ...
        if not (isinstance(result, dict) and result.get("halted")):
            try:
                ...
                logger.exception(
                    "perceived_error_scan failed for workspace %s", workspace_id
                )
