...
def test_agent_service_blocks_explicitly_disabled_agent(workspace_factory, user_factory):
    ...
    on specialists like ``posture_agent`` without manual seeding.
    ...
    ensure_workspace_agent_type(str(workspace.id), agent_type, is_enabled=False, updated_by=owner)
...
@pytest.mark.django_db
def test_agent_service_allows_specialist_without_explicit_entitlement(workspace_factory, user_factory):
    """The 2026-05-08 regression shape, pinned.

    With per-task agent routing, the planner now picks specialists
    like ``posture_agent`` automatically. New workspaces don't have
    ...
    specialists work out of the box. ``ai_teammate_enabled`` and
    ``AgentType.is_active`` still apply as primary controls.
    """
    owner = user_factory()
    workspace = workspace_factory(owner=owner, ai_teammate_enabled=True)
    service = get_agent_service()
    ...
    agent = service.create_agent(
        agent_type="posture_agent",
        user_id=str(owner.id),
        workspace_id=str(workspace.id),
        config={},
    )
    assert agent["agent_type"] == "posture_agent"
...
# NOTE (autosec fork): the sector allow/block-list entitlement tests were removed
# with the ``sectors`` app during the nonprofit-domain strip — sector-driven agent
# gating no longer exists. The remaining primary controls (explicit ``is_enabled``
# opt-out, ``ai_teammate_enabled``, ``AgentType.is_active``) stay covered above.
